...
public class janela extends JFrame implements ActionListener{
    ...
    if (tipoSelecionado != null) {
        ...
        if (diaSelecionado != null) {
            ...
            try {
                Opcao opcao = new Opcao(nomeItem, tipoSelecionado.toLowerCase());
    ...
    for (String dia : diasSemana) {
        ...
        if (!opcoesComida.isEmpty()) {
            for (Opcao comida : opcoesComida) {
                cardapioCompleto.append("- ").append(comida.getNome()).append("\n");
            }
        } else {
        ...
        if (!opcoesBebida.isEmpty()) {
            for (Opcao bebida : opcoesBebida) {
                cardapioCompleto.append("- ").append(bebida.getNome()).append("\n");
            }
        } else {
    ...
    if (diaSelecionado != null) {
        ...
        if (opcoesDia != null && !opcoesDia.isEmpty()) {
            ...
            if (opcaoSelecionada != null) {
                String novoNome = JOptionPane.showInputDialog(this, "Digite o novo nome da opção selecionada:");
                try {
                    for (Opcao opcao : opcoesDia) {
                        if (opcao.getNome().equals(opcaoSelecionada)) {
                            opcao.setNome(novoNome);
                            JOptionPane.showMessageDialog(this, "Opção modificada com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                            return;                        }
                    }
                } catch (NumberFormatException e) {
                    JOptionPane.showMessageDialog(this, "Digite um valor numérico válido para o preço!", "Erro", JOptionPane.ERROR_MESSAGE);
